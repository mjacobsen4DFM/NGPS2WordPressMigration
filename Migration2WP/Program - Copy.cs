...
namespace Migration
{
    class Program
    {
        ...
        static void MigrateImagesOnly(string siteid, string hostUrl, string documents, string temp, string username, string pwd, string destination_siteid, DataSet ds)
        {
            int row = 0;
            foreach (DataRow articleRow in ds.Tables[0].Rows)
            {
                ...
                bool status = BusinessRule.BusinessRule.MigrateImage(hostUrl, username, pwd, destination_siteid, article_uid, "", "N", "7", ref found, ref sent, BusinessRule.BusinessRule.cms.wordpress);
                Console.WriteLine("Row: {0} Migrated Images  Article: {1} status: {2}", row, article_uid, status);
                if (found > 1)
                { // This is a Gallery 
                    status = BusinessRule.BusinessRule.MigrateGallery(siteid, documents, temp, hostUrl, username, pwd, destination_siteid, article_uid, "N");
        ...
        static void Main(string[] args)
        {
            ...
            string hostUrl = BusinessRule.BusinessRule.GetCMSUrl(Convert.ToInt32(siteid), destination_siteid);
            ...
            string pwd = ConfigurationManager.AppSettings["cms_pwd"].ToString();
            ...
            String sqlFormat = System.IO.File.ReadAllText(articlesMissingImagesSQL);
            ...
            string looplog = String.Format("C:\\temp\\TrackLoops_{0}_{1}_{2}.txt", siteid, startdate.Month, startdate.Year);
            int loopcount = 0;
            while (BusinessRule.BusinessRule.BusGetDataset(sqlstatement).Tables[0].Rows.Count > 0)
            {
                ...
                foreach (DataRow articleRow in ds.Tables[0].Rows)
                {
                    ...
                    try
                    {
                        ...
                        status = br.MigrateArticle2WP(taxpubid, siteid, startdate.ToString("yyyyMMdd"), stopdate.ToString("yyyyMMdd"), destination_siteid, documents, temp, hostUrl, username, pwd, article_uid, defaultAuthorTermID, "N", holdrules);
                    ...
                    if (articleTrack != 0 && (articleTrack % 100) == 0)
                    {
                        ...
                        float totaverage = (float)remaining * avg; // Number of seconds remaining
                        TimeSpan t3 = TimeSpan.FromSeconds(totaverage);
                        //Console.WriteLine("Time: {0}, Seconds: {1}, Average: {2}", DateTime.Now.ToString(), seconds, avg);
                        //Console.WriteLine(String.Format("{0}/{1} Remaining Time Est: {2}", articleTrack, total, t3));
                        Console.WriteLine(String.Format("{0}/{1} Remaining Est: {2}, Time: {3}, Seconds: {4}, Average: {5}", articleTrack, total, t3, DateTime.Now.ToString(), seconds, avg));
